...
cfg_model_path = './ObjectDetection/models/yolov5s.pt'
model = None
confidence = .25
...
def image_input(data_src):
    ...
    if data_src == 'Sample data':
        ...
        img_path = glob.glob('./ObjectDetection/data/sample_images/*')
        img_slider = st.slider("Select a test image.", min_value=1, max_value=len(img_path), step=1)
        img_file = img_path[img_slider - 1]
    else:
        img_bytes = st.sidebar.file_uploader("Upload an image", type=['png', 'jpeg', 'jpg'])
        if img_bytes:
            img_file = "./ObjectDetection/data/uploaded_data/upload." + img_bytes.name.split('.')[-1]
...
def video_input(data_src):
    vid_file = None
    if data_src == 'Sample data':
        vid_file = "./ObjectDetection/data/sample_videos/sample.mp4"
    else:
        vid_bytes = st.sidebar.file_uploader("Upload a video", type=['mp4', 'mpv', 'avi'])
        if vid_bytes:
            vid_file = "./ObjectDetection/data/uploaded_data/upload." + vid_bytes.name.split('.')[-1]
...
def get_user_model():
    ...
    if model_src == "file upload":
        ...
        if model_bytes:
            model_file = "./ObjectDetection/models/uploaded_" + model_bytes.name
